...
function fit_rrm_nls(conv::Function=expconv;
...
function fit_rrm_lls(; t::AbstractVector, crr::AbstractVector, ct::AbstractArray, mask=true)
    @assert length(t) == length(crr) == size(ct)[end]
    num_timepoints = length(t)
    if typeof(ct) <: AbstractVector
        @assert length(ct) == num_timepoints
...
function fit_crrm_nls(conv::Function=expconv;
...
function fit_crrm_lls(; t::AbstractVector, crr::AbstractVector, ct::AbstractArray, mask=true, kep_rr=0.0)
    @assert length(t) == length(crr) == size(ct)[end]
    num_timepoints = length(t)
    if typeof(ct) <: AbstractVector
        @assert length(ct) == num_timepoints
...
function fit_errm_lls(; t::AbstractVector, crr::AbstractVector, ct::AbstractArray, mask=true)
    @assert length(t) == length(crr) == size(ct)[end]
    num_timepoints = length(t)
    if typeof(ct) <: AbstractVector
        @assert length(ct) == num_timepoints
...
function fit_cerrm_lls(; t::AbstractVector, crr::AbstractVector, ct::AbstractArray, mask=true, kep_rr=0.0)
    @assert length(t) == length(crr) == size(ct)[end]
    num_timepoints = length(t)
    if typeof(ct) <: AbstractVector
        @assert length(ct) == num_timepoints
...
function fit_cerrm_with_rrift(; crr, cp, t, ct, tail_start, kep_rr=0.0, mask = true)
    ...
    return(estimates=(kt = est.kt, kep = est.kep, ve = est.ve, vp = est.vp, kep_rr = kep_rr, kt_rr = kt_rr, ve_rr = ve_rr), )
end
...
function fit_crrm_with_rrift(; crr, cp, t, ct, tail_start, kep_rr=0.0, mask = true)
    ...
    return(estimates=(kt = est.kt, kep = est.kep, ve = est.ve, kep_rr = kep_rr, kt_rr = kt_rr, ve_rr = ve_rr), )
end
